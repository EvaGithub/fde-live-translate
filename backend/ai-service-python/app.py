...
async def translate_one(text: str, target: str) -> dict:
    ...
    # 1) Ask the cache first (memory → SQLite). A hit means we've already
    #    translated this exact (text, target) — return it WITHOUT calling the LLM.
    ...
    # 2) Miss: call the LLM, then store the result so the next identical request
    #    is a hit. If translate_text raises, we let it propagate (fail loud) so
    #    the gateway returns a 502 — we never store or return untranslated input.
